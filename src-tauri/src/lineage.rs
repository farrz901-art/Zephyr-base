...
fn read_json_file(path: &Path) -> Result<Value, BridgeError> {
    let rendered = fs::read_to_string(path).map_err(|error| {
        BridgeError::lineage(format!(
            "Could not read lineage file {}: {error}",
            path.display()
        ))
    })?;
    serde_json::from_str(&rendered).map_err(|error| {
        BridgeError::lineage(format!(
            "Could not parse lineage file {}: {error}",
            path.display()
        ))
    })
}
...
pub fn read_lineage_snapshot() -> Result<Value, BridgeError> {
    ...
    let public_export_lineage =
        read_json_file(&root.join("manifests/public_export_lineage.json"))?;
    ...
    Ok(json!({
        ...
        "wheelhouse_bundled": root.join("runtime/wheelhouse").exists(),
