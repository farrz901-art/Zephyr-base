...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Validate an offline runtime proof pack result.")
    ...
    proof = _read_json(proof_root / "offline_runtime_proof.json")
    ...
    output_path = (
        proof_root / "offline_runtime_proof_validation.json"
        if proof_root.name == "proof"
        else root / ".tmp/offline_runtime_proof_validation.json"
    )
    report = {
        "schema_version": 1,
        "report_id": "zephyr.base.s14.offline_runtime_proof_validation.v1",
        "summary": {
            "pass": passed,
            "proof_exists": True,
